...
function loadGame(loadgame) {
	...
	for (let i = 0; i < loadKeys.length; i++) { // Iterate over each key in the loadgame object
		if (loadgame[loadKeys[i]] !== undefined) { // Only process keys with defined values
			...
			if (typeof loadgame[thisKey] == "string" && !isNaN(parseFloat(loadgame[thisKey]))) { // If the value is a string that can be parsed as a number, convert it to a Decimal
				...
				for (let j = 0; j < loadgame[thisKey].length; j++) { // Iterate over each element in the array
					if (Array.isArray(loadgame[thisKey][j])) { // If the element itself is an array
						...
						for (let k = 0; k < loadgame[thisKey][j].length; k++) { // Iterate over each element in the nested array
...
let noOfButtons = [multiPrices.length,prestigePrices.length,powerPrices.length,superPrestigePrices.length,ascensionPrices.length,infernalPrices.length];
...
function updateVisuals() {
	...
	if ($('#miningScreen').css('display') == 'block') {
		...
		for (let i=0;i<7;i++) $(".miningResource").eq(i).html(format(game.miningResources[i]));
	}
	
	for (let i=0;i<game.potionCooldowns.length;i++) {
	...
	let XPToNextLevel = levelToXP(game.level.add(1)).sub(levelToXP(game.level));
	let ProgressToNextLevel = (game.XP.sub(levelToXP(game.level)));
	$("#XPBar").css("width", format(ProgressToNextLevel.div(XPToNextLevel).mul(100)) + "%");
}

function updateVariables() {
	let timeDivider = Math.max(1000 / (Date.now() - game.timeOfLastUpdate), 0);
	...
	let potionLastCooldowns = [];
	for (let i=0;i<game.potionCooldowns.length;i++) potionLastCooldowns[i] = game.potionCooldowns[i];
	for (let i=0;i<game.potionCooldowns.length;i++) {
...
function formatTime(x) {
	let timeFloor = Math.floor(x);
	let timeMinutes = Math.floor(timeFloor / 60);
	let timeSeconds = timeFloor % 60;
	let timeString = ((timeMinutes < 10 ? '0' : '') + timeMinutes + ":" + (timeSeconds < 10 ? '0' : '') + timeSeconds + "." + (Math.floor((x % 1) * 100) < 10 ? '0' : '') + Math.floor((x % 1) * 100));
	return timeString;
}

let currentPotionTooltip = 1;
...
function loadWorld(x) {
	...
	if (x==1) {
		...
		for (let i=22;i<noOfButtons[0];i++) {
			button.className = "button button1";
			button.innerHTML = "<p class='buttonText'>Multi:<br>+" + format(new Decimal(multiBoosts[i]).mul(game.prestige.add(1)).mul(game.relicPotionMultipliers[1])) + "<br>Cost:<br>$" + format(multiPrices[i]) + "</p>";
			button.setAttribute("id", i);
			document.getElementById("buttons").appendChild(button.cloneNode(true));
		...
		$("#buttons").append("<br><br><br><h1><span style='color: #60b'>Prestige</span> buttons (<span id='multiHeader' style='color: #b00'>Multi: 0</span>)</h1>");
		for (let i=20;i<noOfButtons[1];i++) {
			button.className = "button button2";
			button.innerHTML = "<p class='buttonText'>Prestige:<br>+" + format(new Decimal(prestigeBoosts[i]).mul(game.power.add(1)).mul(game.relicPotionMultipliers[2])) + "<br>Multi cost:<br>" + format(prestigePrices[i]) + "</p>";
			button.setAttribute("id", i);
			document.getElementById("buttons").appendChild(button.cloneNode(true));
		...
		$("#buttons").append("<br><br><br><h1><span style='color: #dd0'>Power</span> buttons (<span id='prestigeHeader' style='color: #60b'>Prestige: 0</span>)</h1>");
		for (let i=16;i<noOfButtons[2];i++) {
			button.className = "button button3";
			button.innerHTML = "<p class='buttonText'>Power:<br>+" + format(new Decimal(powerBoosts[i]).mul(game.superPrestige.add(1)).mul(game.relicPotionMultipliers[3])) + "<br>Prestige cost:<br>" + format(powerPrices[i]) + "</p>";
			button.setAttribute("id", i);
			document.getElementById("buttons").appendChild(button.cloneNode(true));
		...
		for (let i=10;i<noOfButtons[3];i++) {
			button.className = "button button4";
			button.innerHTML = "<p class='buttonText'>S-prestige:<br>+" + format(new Decimal(superPrestigeBoosts[i]).mul(game.ascension.add(1)).mul(game.relicPotionMultipliers[4])) + "<br>Power cost:<br>" + format(superPrestigePrices[i]) + "</p>";
			button.setAttribute("id", i);
			document.getElementById("buttons").appendChild(button.cloneNode(true));
...
// Debug multiplier for testing purposes
let debugClickMultiplier = 1;

// Set the debug multiplier and update button styles
function setDebugMultiplier(val) {
	debugClickMultiplier = val;
	let btns = document.getElementsByClassName('debug-btn');
	for (let i = 0; i < btns.length; i++) {
		btns[i].style.fontWeight = 'normal';
	}
	document.getElementById('debug-btn-' + val).style.fontWeight = 'bold';
}
...
function buyButton(x, y) {
	if (x == 1) {
		if (game.money.gte(multiPrices[y])) {
			...
			// Applied debugClickMultiplier for testing
			game.multi = game.multi.add(new Decimal(multiBoosts[y]).mul(game.prestige.add(1)).mul(game.relicPotionMultipliers[1]).mul(debugClickMultiplier));
		}
	}
	else if (x == 2) {
		if (game.multi.gte(prestigePrices[y])) {
			...
			// Applied debugClickMultiplier for testing
			game.prestige = game.prestige.add(new Decimal(prestigeBoosts[y]).mul(game.power.add(1)).mul(game.relicPotionMultipliers[2]).mul(debugClickMultiplier));
			game.XP = game.XP.add(1 * debugClickMultiplier);
			checkNextCrateLevel();
		...
		if (game.prestige.gte(powerPrices[y])) {
			...
			// Applied debugClickMultiplier for testing
			game.power = game.power.add(new Decimal(powerBoosts[y]).mul(game.superPrestige.add(1)).mul(game.relicPotionMultipliers[3]).mul(debugClickMultiplier));
			game.XP = game.XP.add(4 * debugClickMultiplier);
			checkNextCrateLevel();
			loadWorld(currentWorld);
		}
	}
		...
		if (game.power.gte(superPrestigePrices[y])) {
			...
			// Applied debugClickMultiplier for testing
			game.superPrestige = game.superPrestige.add(new Decimal(superPrestigeBoosts[y]).mul(game.ascension.add(1)).mul(game.relicPotionMultipliers[4]).mul(debugClickMultiplier));
			game.XP = game.XP.add(16 * debugClickMultiplier);
			checkNextCrateLevel();
			loadWorld(currentWorld);
		}
	}
		...
		if (game.superPrestige.gte(ascensionPrices[y])) {
			...
			// Applied debugClickMultiplier for testing
			game.ascension = game.ascension.add(new Decimal(ascensionBoosts[y]).mul(game.infernal.add(1)).mul(game.relicPotionMultipliers[5]).mul(debugClickMultiplier));
			game.XP = game.XP.add(64 * debugClickMultiplier);
			checkNextCrateLevel();
			loadWorld(currentWorld);
		}
	}
		...
		if (game.ascension.gte(infernalPrices[y])) {
			...
			// Applied debugClickMultiplier for testing
			game.infernal = game.infernal.add(new Decimal(infernalBoosts[y]).mul(game.relicPotionMultipliers[6]).mul(debugClickMultiplier));
			game.XP = game.XP.add(256 * debugClickMultiplier);
			checkNextCrateLevel();
			loadWorld(currentWorld);
		}
	}
...
function unboxItem(x) {
	...
	if (x==1) {
		...
		for (let i=0;i<basicCrateRarities[0].length;i++) totalWeight += basicCrateRarities[0][i];
		for (let i=0;i<basicCrateRarities[0].length;i++) {
		...
		totalWeight = 0;
		for (let i=0;i<basicCrateRarities[itemType].length;i++) totalWeight += basicCrateRarities[itemType][i][1];
		for (let i=0;i<basicCrateRarities[itemType].length;i++) {
		...
		//Determines the type of item to roll for (1 = pattern, 2 = relic, 3 = potion)
		for (let i=0;i<advancedCrateRarities[0].length;i++) totalWeight += advancedCrateRarities[0][i];
		for (let i=0;i<advancedCrateRarities[0].length;i++) {
		...
		totalWeight = 0;
		for (let i=0;i<advancedCrateRarities[itemType].length;i++) totalWeight += advancedCrateRarities[itemType][i][1];
		for (let i=0;i<advancedCrateRarities[itemType].length;i++) {
...
function addItem(x,y,z=0) {
	if (x==0) {
		let listItemPoint = -1;
		for (let i=0;i<game.crates.length;i++) {if (y == game.crates[i][0]) listItemPoint = i;}
		if (listItemPoint == -1) {game.crates.push([y,1]);}
		else {game.crates[listItemPoint][1]++;}
	}
	else if (x==1) {
		let listItemPoint = -1;
		for (let i=0;i<game.patterns.length;i++) {if (y == game.patterns[i][0] && z == game.patterns[i][1]) listItemPoint = i;}
		...
		let listItemPoint = -1;
		for (let i=0;i<game.relics.length;i++) {if (y == game.relics[i][0]) listItemPoint = i;}
		...
		let listItemPoint = -1;
		for (let i=0;i<game.potions.length;i++) {if (y == game.potions[i][0]) listItemPoint = i;}
	...
	if (x==0) {
		...
		if (listItemPoint != -1) {
			if (game.crates[listItemPoint][1] == 1) {
				let listTemp = game.crates.slice(listItemPoint+1);
		...
		if (listItemPoint != -1) {
			if (game.patterns[listItemPoint][2] == 1) {
				let listTemp = game.patterns.slice(listItemPoint+1);
		...
		if (listItemPoint != -1) {
			if (game.relics[listItemPoint][1] == 1) {
				let listTemp = game.relics.slice(listItemPoint+1);
		...
		if (listItemPoint != -1) {
			if (game.potions[listItemPoint][1] == 1) {
				let listTemp = game.potions.slice(listItemPoint+1);
...
function displayItems(x) {
	...
	if (x==1) {
		...
		for (let i=0;i<game.patterns.length;i++) {
			if (game.patterns[i][1] == 2) {itemBox.innerHTML = "<img class='item' style='filter: hue-rotate(240deg)' src='images/pattern" + (game.patterns[i][0] + 1) + "_color.png'><br><p class='itemText'>" + (game.patterns[i][2]) + "</p>";} //blue
			else if (game.patterns[i][1] == 1) {itemBox.innerHTML = "<img class='item' src='images/pattern" + (game.patterns[i][0] + 1) + "_color.png'><br><p class='itemText'>" + (game.patterns[i][2]) + "</p>";} //red
			else {itemBox.innerHTML = "<img class='item' src='images/pattern" + (game.patterns[i][0] + 1) + ".png'><br><p class='itemText'>" + (game.patterns[i][2]) + "</p>";} //black
			itemBox.setAttribute("data-itemnumber", i);
		...
		$("#itemScreenInner").html("");
		for (let i=0;i<game.potions.length;i++) {
...
function checkNextCrateLevel() {
	...
	if (game.level.gte(game.nextCrateLevel) && game.level.lt(100)) {
		...
		let listItemPoint = -1;
		for (let i=0;i<game.crates.length;i++) {if (0 == game.crates[i][0]) listItemPoint = i;}
...
function levelToColour(x) {
	let colour = Math.floor(((x-1) ** 0.5) * 50) % 960;
	let stage = Math.ceil((colour + 1) / 160);

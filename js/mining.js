...
export function updateMiningView(config) {
	...
	setText("minerPurchaseCost", `Costs $${format(game.minerCost)}`);
	setText("miners", format(game.miners));
	setText("miningCooldown", formatTime(game.miningCooldown));
	setText("miningBoost", format(miningMoneyMultiplier(config), 2));
	document.querySelectorAll("[data-ore-id]").forEach(element => {
		const value = format(game.miningResources[config.oreIndexById[element.dataset.oreId]]);
